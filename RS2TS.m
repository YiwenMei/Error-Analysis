...
% - Require TSCls.m, V2DTCls.m, and V3DTCls. If rfT is 'shape', V2DCls.m is also required.
...
addRequired(ips,'Drf',@(x) validateattributes(x,{'V2DTCls','cell'},{'nonempty'},mfilename,'Drf'));
...
if exist(xTfn,'file')~=2 % Extracted time series not existed
  ...
  fprintf('Time series extracted from the target image stack\n');

else
  load(xTfn,'Stg');
  fprintf('Matched time series file loaded\n');
end
...
for s=1:size(Stg,2)
  ...
  ofn=fullfile(opth,sprintf('px_%s_%s.mat',LOI.Sid{s},OStg.vtp));
  save(ofn,'OTS');
  fprintf('%i. Matched "%s" time series pair for station "%s" saved\n',s,OStg.vtp,LOI.Sid{s});
  OTfn=[OTfn;{ofn}];
end
end

% function OTfn=RS2TS_sub2(OStg,OSmk,OSrf,cf,opth,pflg)
